module RoboStat (linearRegression, linearRegressionRSqr) where
...
linearRegressionRSqr xs ys = (alpha, beta, rs)
    where 
...
linearRegression xs = (\(x, y, _) -> (x, y)) . linearRegressionRSqr xs
